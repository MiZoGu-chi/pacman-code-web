...
import model.factories.AgentFactory;
import model.factories.FactoryProvider;
import model.strategies.EscapeStrategy;
import model.strategies.FollowNearestPacmanStrategy;
...
public class PacmanGame extends Game {
    ...
    private static int pointPerFood = 10;
    ...
    public void initializeGame() {
      ...
      /*  pacmans.get(0).setStrategy(new KeyboardStrategy());*/
        ...
        for(PositionAgent p : maze.getGhosts_start()) {
            ...
            ghost.setStrategy(new FollowNearestPacmanStrategy((ArrayList<Pacman>) pacmans));
            ghosts.add(ghost);
    ...
    public void takeTurn() {
        System.out.println("**Pacaman : call takeTurn()**");

        ghostMoveToggle = !ghostMoveToggle;

        for(Ghost g : ghosts) {
            if(g.geStrategy() != null) {
                ...
                System.out.println("Ghost at " + g.getPos() + " plays " + action);
                moveAgent(g, action);
            }
        }
        for(Pacman p : pacmans) {
            if(p.geStrategy() != null) {
                AgentAction action = p.play(maze);
                System.out.println("Pacman at " + p.getPos() + " plays " + action);
                moveAgent(p, action);   
            }         
        }

        if(isFoodEmpty()) {
            System.out.println("PacmanGame : Victory you ate alle the food !");
    ...
    public void moveAgent(Agent agent, AgentAction action) {
        ...
        if(agent instanceof Pacman pacman) {
            ...
            if(maze.isFood(newX, newY)) {
                ...
                /**
                 * règle avec les capsules, qui enclenche un timer de 20 périodes au cours 
    ...
    public void checkCollisions() {
        ArrayList<Pacman> deadPacmans = new ArrayList<>();
        ArrayList<Ghost> deadGhosts = new ArrayList<>();
        ...
        for (Pacman p : pacmans) {
            ...
            for(Ghost g : ghosts) {
                ...
                if(p.getPos().equals(g.getPos())) {
                    System.out.println("PacmanGame : Collision entre un pacman et un fantome");
                    if(p.isInvincible()) {
                        ...
                        // en cas de partie "multi-pacman", ils ont tous un nombre de vie mutualisé
                        if(life <= 0) {
                            p.die();
                            deadPacmans.add(p);
                        }
        ...
        pacmans.removeAll(deadPacmans);
        ghosts.removeAll(deadGhosts);
    }
    ...
    public void setGhostsEscapeStrategy() {
        for(Ghost g : ghosts) {
            g.setStrategy(new EscapeStrategy(pacmans));
        }
    }

    public void setPacmansFollowGhostStrategy() {
        for(Pacman p : pacmans) {
            //p.setStrategy(new FollowNearestGhostStrategy(ghosts));
        }
    }
    ...
    public void resetAllAgentsPostions() {
        for(PositionAgent p : maze.getPacman_start()) {
            AgentFactory factory = FactoryProvider.getFactory(AgentType.PACMAN);
            Pacman pacman = (Pacman) factory.createAgent(p);
            pacmans.add(pacman);
        ...
        for(PositionAgent p : maze.getGhosts_start()) {
            AgentFactory factory = FactoryProvider.getFactory(AgentType.GHOST);
            Ghost ghost = (Ghost) factory.createAgent(p);
            //ghost.setStrategy(new FollowNearestPacmanStrategy(getPacmansPosition()));
            ghosts.add(ghost);
            System.out.println("Ghost created at position: " + p);
        }       
    }
