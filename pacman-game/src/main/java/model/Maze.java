...
public class Maze implements Serializable, Cloneable {
	...
	/**
	 * stocker les états initiaux pour relancer plus facilement un layout sans le recharger
	 * entiérement
	 */
		...
		try {
			...
			while ((ligne = br.readLine()) != null) {
				...
				for (int x = 0; x < ligne.length(); x++) {
					...
					if (ligne.charAt(x) == '.') {
						food[x][y] = true;
						initialFood[x][y] = true;
					}
					else
						food[x][y] = false;
					if (ligne.charAt(x) == 'o') {
						capsules[x][y] = true;
						initialCapsules[x][y] = true;
					}
					else
						capsules[x][y] = false;
					if (ligne.charAt(x) == 'P') {
	...
	public boolean isLegalMove(Agent agent, AgentAction action) {
        ...
        if (newX < 0 || newX >= getSizeX() || newY < 0 || newY >= getSizeY()) {
            return false;
        }
	...
	// nouvelle méthode pour remettre les élements du labyrinte à leurs états initiaux
	public void resetFoodsAndCapsules() {
		for (int x = 0; x < size_x; x++) {
			for (int y = 0; y < size_y; y++) {
				this.food[x][y] = this.initialFood[x][y];
				this.capsules[x][y] = this.initialCapsules[x][y];
			}
		}
	}
}